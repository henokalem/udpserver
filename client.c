...
char* requestString(long *length, char* filename)
{
	char * buffer = 0;
...
*/
  ...
  char* buffer = requestString(length, filename);

  //char buffer[18] = "This is my message";

  const int datasize = *length;	/* data buffer size */
  ...
  if (argc != 6)		/* Test for correct number of arguments */
    {
      fprintf (stderr,
	       "Usage: %s <Server IP> <Server Port No> <Chunk size> <Window Size> <filename>\n",
	       argv[0]);
      exit (1);
    }
